...
locals {
  ...
  service_name               = "company-links-consumer"
  container_port             = "8080"
  docker_repo                = "company-links-consumer"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path           = "/company-links-consumer/healthcheck" #healthcheck path for company-links-consumer
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "company-links-consumer.env"
