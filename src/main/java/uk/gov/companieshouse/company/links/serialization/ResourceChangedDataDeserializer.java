...
public class ResourceChangedDataDeserializer implements Deserializer<ResourceChangedData> {
    ...
    public ResourceChangedData deserialize(String topic, byte[] data) {
        try {
            ...
            var resourceChangedData = reader.read(null, decoder);
            logger.trace(String.format("DSND-374: Message successfully de-serialised into "
                    + "Avro ChsDelta object: %s", resourceChangedData));
