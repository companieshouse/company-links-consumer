...
public class ChargesStreamRetryStepDefs {
    ...
    public void stubbed_company_profile_api_endpoint_will_return_http_response_code(Integer statusCode) {
        WiremockTestConfig.stubGetCompanyProfile(companyNumber,statusCode, "profile-with-null-attribute");
    }
    ...
    public void the_message_is_successfully_consumed_only_once_from_the_topic(String topicName) {
        assertThatThrownBy(() -> KafkaTestUtils.getSingleRecord(kafkaConsumer, topicName)).hasStackTraceContaining("No records found for topic");
    }
    ...
    public void failed_to_process_and_immediately_moved_the_message_into_topic(String topicName) {
        failed_to_process_and_immediately_sent_the_the_message_into_topic_for_attempts(0, topicName );
    }
    ...
    public void a_valid_message_in_avro_format_message_with_an_invalid_url_format_is_sent_in_resource_uri_attribute_to_the_kafka_topic(String topicName)
            throws InterruptedException {
    ...
    public void stubbed_company_profile_api_patch_endpoint_will_return_bad_request_http_response_code(Integer statusCode) {
        WiremockTestConfig.stubGetCompanyProfile(companyNumber,statusCode, "profile-with-insolvency-links");
    ...
    public void stubbed_company_profile_api_patch_endpoint_throws_an_internal_exception() {
        WiremockTestConfig.stubGetCompanyProfile(companyNumber,HttpStatus.OK.value(), "profile-with-insolvency-links");
        WiremockTestConfig.stubPatchCompanyProfile(companyNumber,HttpStatus.INTERNAL_SERVER_ERROR.value());
    }

    @When("A valid Avro message with valid json payload is sent to the Kafka topic {string} topic")
    public void a_valid_avro_message_with_valid_json_payload_is_sent_to_the_kafka_topic(String topicName)
            throws InterruptedException {
    ...
    public void stubbed_company_profile_api_get_endpoint_is_down() {
        WiremockTestConfig.stubGetCompanyProfile(companyNumber, HttpStatus.SERVICE_UNAVAILABLE.value(), "");
    }
    ...
    public void failed_to_process_and_immediately_sent_the_the_message_into_topic_for_attempts(Integer numberOfAttempts, String topicName) {
